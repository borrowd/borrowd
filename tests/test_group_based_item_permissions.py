...
class GroupBasedItemPermissionsTests(TestCase):
    ...
    def test_item_not_visible_to_member_of_unselected_group(self) -> None:
        # Arrange
        owner = self.owner
        member = self.member

        group_a: BorrowdGroup = BorrowdGroup.objects.create_group(
            name="Group A",
            created_by=owner,
            updated_by=owner,
            membership_requires_approval=False,
        )
        group_b: BorrowdGroup = BorrowdGroup.objects.create_group(
            name="Group B",
            created_by=owner,
            updated_by=owner,
            membership_requires_approval=False,
        )
        group_b.add_user(member)
        ...
        # Assert
        ## Member is in group_b but item is only shared with group_a.
    ...
    def test_all_items_visible_to_group_by_default(self) -> None:
        # Arrange
        owner = self.owner
        ...
        # Assert — all items default to share_with_all_groups=True
        self.assertIn(ItemOLP.VIEW, get_perms(perms_group, item1))
        self.assertIn(ItemOLP.VIEW, get_perms(perms_group, item2))
        self.assertIn(ItemOLP.VIEW, get_perms(perms_group, item3))

    def test_item_not_visible_to_group_when_not_selected(self) -> None:
        # Arrange
        owner = self.owner
        ...
        perms_group_a = group_a.perms_group
        perms_group_b = group_b.perms_group

        # Act — items shared only with group_a
        item1 = Item.objects.create(
            name="Test Item 1",
            owner=owner,
            created_by=owner,
        ...
        item1.shared_with_groups.set([group_a])
        ...
        item2.shared_with_groups.set([group_a])

        # Assert
        self.assertIn(ItemOLP.VIEW, get_perms(perms_group_a, item1))
        self.assertIn(ItemOLP.VIEW, get_perms(perms_group_a, item2))
        self.assertNotIn(ItemOLP.VIEW, get_perms(perms_group_b, item1))
        self.assertNotIn(ItemOLP.VIEW, get_perms(perms_group_b, item2))

    def test_item_visibility_revoked_when_sharing_restricted(self) -> None:
        # Arrange
        owner = self.owner
        ...
        # Assert initial visibility (share_with_all_groups=True by default)
        self.assertIn(ItemOLP.VIEW, get_perms(perms_group, item))

        # Act — restrict sharing to no groups
        item.share_with_all_groups = False
        item.updated_by = owner
        item.save()
        item.shared_with_groups.clear()
    ...
    def test_item_visibility_granted_when_sharing_expanded(self) -> None:
        ...
        # Assert initial visibility (item not shared with any group)
        self.assertNotIn(ItemOLP.VIEW, get_perms(perms_group, item))

        # Act — expand sharing to all groups
        item.share_with_all_groups = True
        item.updated_by = owner
        item.save()
